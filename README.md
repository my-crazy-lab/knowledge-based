# Books

- High Performance Browser Networking
    - 1 :arrow_right: 21
- The Web Application Hacker's Handbook
    - [1 :arrow_right: 368](./1.0.0/siqgx4.md)
- The Architecture of Open Source Applications volume 1
    - [Chapter 3](./1.0.0/kep0z3.md)
- Linux from scratch book
    - 1 :arrow_right: 30
- Patterns of Enterprise Application Architecture
    - 1 :arrow_right: 1
- [Site Reliability Engineering](https://sre.google/sre-book/table-of-contents/)
    - 1 :arrow_right: 20 Chapter 10

- Speech and Language Processing
    - [1 :arrow_right: 400](./1.0.0/x7r2ri.md)
- The Data Warehouse Toolkit, 3rd Edition
    - 1 :arrow_right: 37
- Data Science for Business
    - [48 :arrow_right: 67](./1.0.0/cxn9wz.md)
    - [68 :arrow_right: 96](./1.0.0/s2nnpb.md)
    - [97 :arrow_right: 119](./1.0.0/inkjdo.md)
- [Foundations of Computer Vision](https://visionbook.mit.edu/) 
- [Computer Graphics from scratch](https://www.gabrielgambetta.com/computer-graphics-from-scratch/00-introduction.html) 

## Solved

- :white_check_mark: Infrastructure as Code: Managing Servers in the Cloud
    - 1 :arrow_right: DONE
- :white_check_mark: Systems Performance: Enterprise and the Cloud
    - 1 :arrow_right: DONE
- :x: The Art of Scalability
    - 1 :arrow_right: 116
- :white_check_mark: Obserbility Engineering
    - 1 :arrow_right: DONE
- :white_check_mark: Designing Data-Intensive Applications
    - [91 :arrow_right: 106](./1.0.0/tkzqg5.md)
    - [107 :arrow_right: 222](./1.0.0/4mw4a3.md)
    - [222 :arrow_right: 295](./1.0.0/o70doc.md)
    - [295 :arrow_right: 371](./1.0.0/n20071.md)
    - [372 :arrow_right: 410](./1.0.0/o6n1x2.md)
    - 410 :arrow_right: DONE
- :white_check_mark: The Unicorn Project
    - 1 :arrow_right: DONE
- :white_check_mark: The Phoenix Project
    - 1 :arrow_right: DONE
- :white_check_mark: Software Architecture The Hard Parts
    - [1 :arrow_right: DONE](./1.0.0/3pm1sl.md)
- :white_check_mark: (Little OS book from build own X)[https://littleosbook.github.io/]
- :white_check_mark: Company of One
- :white_check_mark: Make: Bootstrapper's handbook
- :white_check_mark: The Clean startup
- :white_check_mark: Machine Learning Engineering
- :white_check_mark: deep-learning-for-vision-systems
    - [1 :arrow_right: DONE](./1.0.0/sfhs65.md)
- :white_check_mark: THE STARTUP CTO'S HANDBOOK
    - 1 :arrow_right: DONE
- :white_check_mark: document software architecture
    - [1 :arrow_right: 250](./1.0.0/7si7e0.md) Not need read.
- :white_check_mark: Introducing MLOps
    - [1 :arrow_right: DONE](./1.0.0/qbmjfb.md)
- :white_check_mark: Building Microservices
    - 1 :arrow_right: DONE
- :white_check_mark: Solution Architecture handbook
    - [1 :arrow_right: DONE](./1.0.0/bsyxxk.md) Defining the modernization approach
- :white_check_mark: Learning Domain-driven design
    - [29 :arrow_right: 58](./1.0.0/hxzy09.md)
    - [59 :arrow_right: 86](./1.0.0/qqo5s7.md)
    - [87 :arrow_right: 162](./1.0.0/sohwje.md)
    - [163 :arrow_right: 201](./1.0.0/j4kegr.md)
    - [202 :arrow_right: done](./1.0.0/c7dq63.md)
    - [Summary](./1.0.0/wfu2jf.md)
- :white_check_mark: Prompt Engineering for LLMs
    - [1 :arrow_right: DONE](./1.0.0/u57umk.md)
- :white_check_mark: Machine Learning Yearning (Vi)
    - [1 :arrow_right: 60](./1.0.0/w3y60i.md)
- :white_check_mark: Multiplayer Game Programming: Architecting Networked Games
    - [1 :arrow_right: Done](./1.0.0/d17bqf.md)
- :white_check_mark: eCommerce in the Cloud
    - [1 :arrow_right: DONE](./1.0.0/jmzd7s.md) chapter 11
- :white_check_mark: gemini for google workspace prompting guide 101
    - 1 :arrow_right: DONE
- :white_check_mark: DevOps Handbook
- :white_check_mark: The Analytics Setup Guidebook
- :white_check_mark: Generative AI in practice: 100+ amazing ways

# Mobile

## React Native

- **`mdzu7u`**
    - [Performance Overview](https://reactnative.dev/docs/performance)
    - [JavaScript Environment](https://reactnative.dev/docs/performance)
    - [Native platform](https://reactnative.dev/docs/native-platform)

## Metro

- **`bly5bk`**
    - [Metro docs](https://metrobundler.dev/docs/bundling): Bundle, caching, module resolution, source map format

# Web

## React

- **`522nan`**
    - [Update some new hooks](https://react.dev/reference/react/hooks)
- **`j85yn5`**
    - [Some news api react](https://react.dev/reference/react/apis)
    - [Some news api react-dom](https://react.dev/reference/react-dom)

# Server (BE)

## Nodejs

- **`y5s9eb`**
    - [Compare web stream and nodejs stream](https://betterstack.com/community/guides/scaling-nodejs/nodejs-streams-vs-web-streams-api/)

# Database

## SQL
- [Anatomy Of A SQL Engine](./1.0.0/zc8ms2.md)

# English

## Vocabulary

- [27 04 25](./1.0.0/yagy18.md)
- [30 04 25](./1.0.0/d9vx1k.md)
- [07 05 25](./1.0.0/e1jj8d.md)

# Hard working

- [Trí tuệ nhân tạo nâng cao](./1.0.0/qrw1qq.md)

# Job hopping

## Database

- [Thiết kế Physical Schema chịu tải cao](./1.0.0/c2gn4i/README.md)
- Sharding kiến trúc Horizontal Scaling	
    - PostgreSQL: pg_shard, Citus
    - MongoDB: shard key, zone
    - Định nghĩa tiêu chí chọn shard key tốt (low cardinality vs high skew)	
    - Đảm bảo khả năng scale out, giảm bottleneck khi 1 node vượt quá khả năng chịu tải
- Tối ưu transaction & concurrency control
    - PostgreSQL: SERIALIZABLE vs REPEATABLE READ
    - Deadlock simulation
    - Advisory locking vs pessimistic locking
    - Xử lý conflict, đảm bảo tính toàn vẹn dữ liệu khi concurrent transactions tăng cao
- Replication & High Availability Design
    - Triển khai replica cluster (1 primary - n replica)
    - Test failover tự động- Monitor replication lag với pg_stat_replication, mongostat	
    - Tăng độ sẵn sàng (HA), hỗ trợ scale đọc, bảo vệ production khi node hỏng
- Write-heavy vs Read-heavy Load Simulation
    - Dùng pgbench, sysbench, hoặc custom load tool
    - Benchmark Read QPS vs Write TPS
    - Tối ưu bằng index, cache, queue, batch write	
    - Phân tích hành vi hệ thống dưới các dạng tải, đưa ra chiến lược phân phối load
- Hybrid: SQL + NoSQL trong cùng hệ thống
    - PostgreSQL (OLTP) cho transactional
    - MongoDB/Redis cho content/document cache
    - Xây bridge bằng app layer hoặc ETL (CDC)	
    - Thiết kế kiến trúc polyglot persistence – chọn đúng DB cho đúng use case
- Zero-Downtime Schema Migration Strategy
    - Flyway/Liquibase versioning
    - Add column → backfill → swap
    - Rollback design, dark launch kiểm thử	
    - Là kỹ năng bắt buộc khi deploy DB ở môi trường production với downtime bằng 0
- Audit Logging + Change Data Capture (CDC)
    - PostgreSQL Logical Replication
    - MongoDB Oplog tailing
    - Debezium Kafka connector
    - Build event-sourcing for DB events	
    - Hỗ trợ audit, phân tích dữ liệu theo thời gian, hoặc xây hệ thống realtime downstream từ dữ liệu DB
- Schema Evolution trong Microservices
    - Chia DB theo bounded context
    - Event outbox pattern
    - DB backward compatibility + integration tests	
    - Giải quyết bài toán DB chia nhỏ (microservices) nhưng vẫn đảm bảo tính nhất quán và forward-compatibility
- Thiết kế chiến lược backup & disaster recovery
    - Point-in-time recovery (PITR)
    - Restore test automation
    - Tính toán RPO/RTO	
    - Đảm bảo không mất dữ liệu khi thảm họa (kỹ thuật hay con người), critical ở tầm architect
- So sánh CAP: MongoDB vs PostgreSQL trong môi trường production
    - Viết 2 use-case yêu cầu độ nhất quán khác nhau
    - Thử partition tolerance, replication inconsistency
    - Kết luận thiết kế	
    - Áp dụng tư duy kiến trúc hệ thống phân tán thực tế – hiểu rõ giới hạn từng hệ quản trị
- Tích hợp Redis làm write-through + fallback cache layer
    - Viết code logic fallback từ Redis → DB
    - Invalidate theo TTL hoặc pub/sub
    - Đo hiệu quả với latency & hit ratio	
    - Tăng tốc hệ thống backend, đảm bảo graceful degradation khi cache lỗi
- Indexing Strategy & Query Plan Optimization
    - Phân tích query execution plan (PostgreSQL `EXPLAIN ANALYZE`, MongoDB `explain()`).
    - Tối ưu composite index, covering index, partial index.
- Multi-region Deployment Strategy
    - Active-active vs Active-passive DB deployments.
    - Latency mitigation & replication lag handling (especially with write-heavy systems).
- Data Lifecycle & Archival Strategy
    - Cold/hot storage strategy.
    - TTL (time-to-live), partitioning cho table quá lớn.
- Security & Compliance
    - Field-level encryption, at-rest vs in-transit encryption.
    - Row-level security (PostgreSQL RLS).
    - GDPR/Data retention strategies.
- Observability & Telemetry cho Database
    - Slow query logs, deadlock monitoring, replication lag alerts.
    - Thống kê database health (pg_stat_activity, Mongo Atlas metrics...).
- Distributed Transactions / Sagas / Eventual Consistency
    - Nếu bạn dùng microservices, thì nên hiểu rõ:
        - Sagas (choreography vs orchestration).
        - Outbox Pattern + Relay + Debezium.
        - Idempotency Key Handling.
- Schema Versioning Tooling
    - Liquibase, Flyway, Sqitch – so sánh & lựa chọn phù hợp.
- Cost Optimization (Cloud DB Services)
    - Chi phí read replica, long-term storage, IOPS-based pricing (RDS/Aurora).
    - Tối ưu hóa schema và indexes để tiết kiệm chi phí trên cloud.

## API - HTTP
- [Xây dựng REST API chuẩn cấp Production cho hệ thống lớn](./1.0.0/9w5kku.md)
- Tối ưu hóa performance qua kỹ thuật HTTP	
    - GZIP compression- Keep-Alive & connection reuse- HTTP/2 multiplexing- Giảm round-trip (n+1 calls, eager fetching)	Tối ưu throughput và latency khi backend phục vụ >1000 request/giây
API Rate Limiting và Throttling	- Triển khai rate limit bằng middleware hoặc reverse proxy (nginx, envoy)- Thử brute force script- Hiển thị 429 Too Many Requests + Retry-After	Bảo vệ API khỏi abuse, đảm bảo fairness cho người dùngQuan trọng trong môi trường production
OpenAPI (Swagger) + API Documentation Lifecycle	- Viết API Spec bằng OpenAPI 3.0- Tự động tạo client SDK (e.g., OpenAPI Generator)- Kiểm thử contract bằng Postman / Dredd	Đảm bảo client/backend đồng bộ, giảm bug integrationBắt buộc cho microservices
API Gateway: Centralized routing + security layer	- Sử dụng Kong / Traefik / NGINX- Add layer: Auth, request transformation, caching- So sánh với BFF (Backend for Frontend)	Thiết kế tập trung bảo mật, route control, và auditGiúp tổ chức API tốt hơn trong hệ thống lớn
Authentication & Authorization (JWT + OAuth2)	- Implement OAuth2 Authorization Code Flow- Tách Auth service- Ký + validate JWT + refresh token	Kiến thức bảo mật API, ngăn giả mạo và CSRFThiết yếu trong mọi sản phẩm sản xuất
Tách biệt command vs query (CQRS cho API)	- GET dùng 1 route chuyên biệt- POST/PUT không trả object- Tách write/read model cho scale	Giảm coupling giữa phần ghi + phần đọc, tối ưu hiệu suấtPhù hợp kiến trúc phức tạp
Xây dựng retry, idempotency và timeout cho API client	- Retry x3 với backoff- Thiết kế API PUT/DELETE idempotent- Thử lỗi mạng (chaos testing)	Bắt buộc để tăng tính ổn định trong microservice hoặc API publicGiảm tỉ lệ lỗi do call trùng
Cấu hình reverse proxy / API gateway caching layer	- NGINX cache + Cache-Control + Vary- CDN-aware (Cloudflare headers)- Cache Invalidation strategy	Giảm tải hệ thống backend + tăng tốc response timeĐặc biệt quan trọng khi traffic cao
Cross-Service Communication: REST + gRPC comparison	- Triển khai REST API nội bộ- Triển khai song song với gRPC- Benchmark latency, size, schema strictness	Phân biệt khi nào nên dùng REST và khi nào nên chuyển sang gRPC hoặc event-driven
Security Audit REST API (OWASP Top 10 API)	- Kiểm tra: Broken Auth, Mass assignment, Rate limiting- Sử dụng tools như OWASP ZAP / Postman Fuzzer	Tránh lỗ hổng nghiêm trọng trong REST APIsBắt buộc ở môi trường fintech / health / public API
Logging & Trace REST Call Chains Across Services	- Thêm correlation ID- Tích hợp OpenTelemetry hoặc custom trace ID qua header- Xem lại trace log thông qua Jaeger hoặc Zipkin	Quan sát được performance toàn hệ thống phân tánDễ debug và optimize

## Caching
- [Cache-aside với Redis cho REST API](./1.0.0/q0h2zj.md)
- Write-through cache cho transactional data	
    - Thử mô hình ghi đồng thời vào DB + cache- Kiểm tra cache consistency khi DB lỗi- Benchmark độ trễ write	Phân biệt write-through & write-backỨng dụng cho dữ liệu đọc nhiều - ghi ít
Invalidate cache thông minh khi dữ liệu thay đổi	- Xác định chiến lược: TTL, Manual Invalidation, Pub/Sub- Dùng Redis Pub/Sub hoặc webhook để xoá cache liên quan	Tránh stale cache gây lỗiRất quan trọng với hệ thống phân tán và dữ liệu động
CDN Caching + Edge rules (Cloudflare, Fastly)	- Tạo rule caching dựa trên URL, headers- Kiểm thử cache purging- Tối ưu latency cho static/HTML pages	Thực tế cho web-scale systemGiảm chi phí server và tăng tốc độ toàn cầu
Implement Content Negotiation + Vary caching	- Dùng Vary: Accept-Language, Vary: User-Agent- Cache đa phiên bản response	Tránh xung đột cache khi user agent hoặc ngôn ngữ khác nhau
Query Result Caching với Redis / Memcached	- Cache kết quả các truy vấn SQL tốn tài nguyên- So sánh hiệu năng khi không dùng cache	Giảm áp lực lên DB, scale được với read-heavy workload
Level-based Caching Strategy: Local, App-layer, CDN	- Cache multi-layer:→ App memory (in-memory LRU)→ Redis→ CDN- Đo performance gain từng layer	Kiến thức hệ thống đa tầngBắt buộc khi scaling đến > 10K request/giây
Cache Stampede Protection (e.g., Mutex, Request Coalescing)	- Dùng lock/mutex để ngăn 1000 requests cùng lúc query DB khi cache hết hạn- Implement coalescing/circuit breaker	Tránh "cache stampede" gây overload DBKỹ thuật cần thiết khi TTL ngắn và nhiều concurrent user
Cache Metrics & Observability (Prometheus, Grafana)	- Expose cache hit/miss ratio- Đếm eviction, TTL expiry, memory usage- Alert nếu cache hit rate < 80%	Đo lường hiệu quả hệ thốngPhát hiện sớm vấn đề performance do cache không hiệu quả
Build Cache Consistency Test Suite	- Viết test kiểm tra cache sync khi update- Test cho race condition + concurrent update- Measure stale duration	Tránh bugs âm thầm do cache saiCực kỳ quan trọng cho các hệ thống tài chính hoặc real-time data
Design TTL Strategy Per Data Domain	- Phân tích loại dữ liệu (hot/cold)- Gán TTL tương ứng- Implement adaptive TTL dựa trên access pattern	Hiểu sâu về access pattern + data lifecycleTối ưu cost và độ chính xác dữ liệu
Compare Cache Backends: Redis vs Memcached vs Local	- Triển khai từng loại- Đo latency, memory footprint, eviction policy- So sánh use-case phù hợp từng loại	Biết chọn công cụ phù hợp cho từng bài toánPhân biệt rõ ràng ưu/nhược điểm

## Message Queue

- [Producer-Consumer với RabbitMQ](./1.0.0/4qkfz8.md)
- Dead Letter Queue (DLQ)	
    - Định nghĩa retry + dead-letter exchange- Tạo rule: nếu xử lý lỗi → chuyển qua DLQ- Log & monitor DLQ	Xử lý thất bại đúng cách, không làm "chết" hệ thốngBắt buộc cho các hệ thống reliable
Retry Logic + Backoff (exponential retry)	- Gửi message lỗi vào retry queue- Tăng thời gian chờ theo số lần thất bại- So sánh fixed delay vs exponential backoff	Tránh hệ thống bị "hammered" bởi retry loopTối ưu resilience
Exactly-once vs At-least-once vs At-most-once Delivery	- Viết consumer xử lý giao dịch tài chính- Tạo thử 2 loại lỗi: duplicate + mất message- Thử các cơ chế Idempotent handling	Phân biệt delivery semantics, critical khi làm data processing hoặc tài chính
Scaling Consumer Groups (Kafka)	- Setup Kafka Cluster (Docker)- Dùng nhiều consumer group- Quan sát load balancing, partition assignment	Hiểu mô hình scale-out streaming và fault tolerance của Kafka
Event Sourcing via Message Queue	- Thiết kế service ghi log action qua MQ- Replay lại log để rebuild state- So sánh với CRUD truyền thống	Phục vụ mục tiêu audit, debugging, state recreationGần với kiến trúc CQRS, ES
FIFO và Priority Queue	- Dùng RabbitMQ FIFO + Priority plugin- Gửi message có level ưu tiên- Đảm bảo xử lý đúng thứ tự và ưu tiên trước	Thường dùng trong hàng đợi task quan trọngVí dụ: task khách VIP xử lý sớm hơn
Build Audit Trail từ MQ	- Ghi lại toàn bộ message qua MQ thành audit log- Gắn correlation ID- Replay hoặc debug theo ID	Đảm bảo traceabilityGóc nhìn observability cao trong các system critical
Batch Message Processing + Parallel Consumer	- Consumer xử lý message theo batch (ví dụ: 10/lượt)- Đo hiệu suất và xử lý parallel	Tối ưu hiệu suất xử lý, giảm chi phíHạn chế overload khi message đến quá nhanh
Distributed Transaction Simulation (Outbox pattern)	- Triển khai Outbox table trong DB- Producer đọc từ DB → gửi vào MQ- Đảm bảo eventual consistency	Giải pháp chống lost-message trong transactional systemYêu cầu hiểu system boundaries
Chaos Testing Message Loss + Latency	- Dùng tools để drop/late message (toxiproxy, tc)- Kiểm tra cách service phục hồi- Log & phân tích ảnh hưởng	Đo resilience hệ thống khi MQ gặp sự cốEssential cho production-grade system
Monitoring MQ Metrics (Lag, Throughput, Retry Rate)	- Dùng Prometheus + Grafana hoặc quản lý RabbitMQ plugin- Cảnh báo nếu retry rate > ngưỡng- Visualize throughput theo thời gian	Biết điểm nghẽn, queue đầy, retry tốn tài nguyênTối ưu hệ thống real-time
Integration giữa Service A → MQ → Service B (thực tế)	- Tạo chuỗi xử lý: Service A gửi qua MQ, B nhận và phản hồi lại- Gắn tracing ID xuyên suốt	Cốt lõi trong microservice và event-drivenHiểu rõ flow của 1 business event end-to-end
Compare: RabbitMQ vs Kafka vs SQS vs NATS (theo use-case)	- So sánh latency, ordering, message loss, persistence- Benchmark cho 1 use-case cụ thể	Biết chọn công nghệ đúng cho bài toánRabbitMQ cho transaction, Kafka cho streaming, NATS cho low-latency

## Clean Arch, Patterns

- [Implement Clean Architecture (Onion/Hexagonal)](./1.0.0/6e1imr.md)
- Dependency Inversion Principle (DIP)	
    - Tạo Interface + các Adapter cụ thể- Dùng IoC Container (Inversify, Spring, NestJS)	Thực hành DIP - nền tảng của Clean Architecture
Repository Pattern	- Tạo 2 backend khác nhau (InMemory vs Postgres)- Repository interface trung gian- Viết unit test độc lập DB	Cho phép mocking dễ dàng, tách business logic khỏi data source
Command Query Responsibility Segregation (CQRS)	- Phân chia handler Read vs Write riêng biệt- Write → DB, Read → cache- Tối ưu scale và read performance	Áp dụng cho hệ thống scale lớn hoặc write-heavy vs read-heavy
Event-driven domain logic (Domain Events)	- Trigger event trong domain (ex: OrderCreated)- Event handler xử lý logging, email, async ops- Dùng Event Bus hoặc simple pub/sub	Tách biệt domain logic + cross-cutting concern
Service Layer vs Application Layer phân biệt rõ	- Viết use case trong Application Layer- Controller gọi UseCase thay vì Business Logic	Thể hiện đúng architecture intentionBusiness logic không dính vào Web/DB/IO
Builder Pattern cho object phức tạp (Report, Invoice...)	- Tạo một report với nhiều config (CSV, PDF...)- Dùng Builder pattern để generate object	Tránh constructor dài, tăng readability
Factory + Strategy Pattern	- Factory: tạo instance của PaymentMethod (PayPal, Stripe)- Strategy: thay đổi logic vận chuyển, giảm giá...	Thực hành lựa chọn động theo runtime behavior
State Pattern (ex: Order Status, Workflow)	- Triển khai Order với state transition (Pending → Paid → Shipped)- Mỗi state có behavior riêng	Tăng khả năng mở rộng logic theo từng trạng thái
Specification Pattern (kiểm tra Rule phức tạp)	- Tách điều kiện (rules) phức tạp thành Specification object- Compose bằng AND, OR, NOT	Dễ dàng test rule, reuse, combine logic
Decorator Pattern	- Decorate handler bằng logging, retry, auth...- Không thay đổi code logic chính	Tách biệt concern theo đúng SOLIDDễ mở rộng mà không sửa lớp chính
Use Case: Viết Service dùng Event-Driven + Clean Architecture	- Build OrderService theo Clean Arch- Event: OrderCreated → Email → Update Inventory- Domain logic chỉ publish Event, không call service trực tiếp	Xây hệ thống loosely coupledReady cho microservices hoặc serverless
Anti-pattern Review (Anemic Domain, God Service...)	- Refactor 1 project có Business Logic dính controller- Đưa domain logic về UseCase hoặc Domain Entity	Tránh lỗi phổ biến làm hệ thống khó mở rộng và test
Plugin Architecture (Sử dụng interface mở rộng feature)	- Build core feature, thêm plugin xử lý mới (ex: thêm validation, thêm loại thanh toán mới...)	Dễ mở rộng sản phẩm, scale theo team, publish như SDK
DDD Tactical Patterns (Aggregate, ValueObject, Entity, Domain Event)	- Dùng DDD trong hệ thống đặt hàng- Phân biệt Aggregate vs Entity vs VO- Bảo vệ consistency boundary	Hiểu sâu kiến trúc theo Domain-Driven DesignYêu cầu tại cấp Principle

## cloud + CI/CD

- [CD Pipeline: Auto deploy dev/staging với feature branch, tạo Dockerfile chuẩn production, tích hợp Secret Management](./1.0.0/9luvs1.md)
- IaC với Terraform (AWS/GCP)	
    - Viết file tạo S3, EC2, EKS, IAM
    - Apply, destroy, versioned infra	
    - Kiểm soát hạ tầng dưới dạng code, Tăng tính tái sử dụng và kiểm tra
- Triển khai ứng dụng lên Cloud (AWS/GCP)	- Dùng ECS/EKS (AWS) hoặc GKE (GCP)- Setup service autoscaling- Deploy thông qua CI/CD	Xây hệ thống cloud-native, tự mở rộngRất cần ở cấp Principle
Blue/Green hoặc Canary Deployment	- Dùng ArgoCD hoặc Spinnaker hoặc GitOps- Deploy theo tỷ lệ (ex: 10%, 50%, 100%)- Monitor rollback	Triển khai an toàn, rollback dễ dàngGiảm downtime
Self-healing Infra (Auto restart + Alerting + Healthcheck)	- Kubernetes liveness/readiness probe- Auto-restart pod nếu crash- Alert qua Slack/email nếu lỗi build/deploy	Hệ thống resilient, tự chữa lỗi
Branch Strategy + Tagging (SemVer, Release Flow)	- Dùng versioning chuẩn: v1.2.3- Auto tag khi merge vào main, release note- Phân nhánh chuẩn CI/CD	Quản lý release rõ ràng, dễ rollback
Build & deploy Microservices + Central Config + Shared Secrets	- Multiple services sharing secrets- Sử dụng Consul/SSM/Vault cho config centralized- Chia pipeline theo service	Quản lý pipeline microservices
GitOps với ArgoCD hoặc Flux	- Source of truth là Git- ArgoCD theo dõi & sync tự động Kubernetes state	CI đẩy image, CD theo dõi Git để applyÁp dụng rộng trong Cloud Native teams
Test CI/CD Failure & Recovery	- Tạo lỗi cố ý trong build/test- Kiểm tra rollback auto, alert, và fix nhanh chóng	Huấn luyện tinh thần “fail fast, fix faster”
Set up CI/CD cho monorepo vs polyrepo	- Nhiều project/package- Trigger từng module riêng- Chia pipeline theo folder	Đáp ứng cho hệ thống lớn, đa dự án
Implement Audit Trail cho Deployment	- Ghi log lịch sử deploy- Ai deploy? Deploy gì? Bao giờ?	Hệ thống chuẩn enterprise luôn yêu cầu audit
Auto rollback khi deploy fail / healthcheck fail	- Implement logic revert commit hoặc rollback deploy- Monitor CPU/health nếu vượt ngưỡng → revert	Tối ưu uptime & tránh ảnh hưởng user

## Monitoring & Logging

- [Tích hợp log tập trung với ELK / Loki](./1.0.0/44enpa.md)
- Set up Prometheus + Grafana để monitor app metrics	
    - Tạo custom metrics (http_requests_total, latency, v.v.)- Export metric endpoint- Visualize với Grafana	Theo dõi hiệu suất realtime
Alerting qua Slack / Email / PagerDuty	- Cấu hình cảnh báo Prometheus Alertmanager hoặc Grafana Alert- Rule: 5xx rate tăng, memory leak, disk full	Phản ứng nhanh với sự cố
Tracing request bằng OpenTelemetry / Jaeger	- Trace xuyên suốt microservices- Xác định bottleneck hoặc service gây lỗi- Kết hợp với Grafana Tempo hoặc Jaeger	Critical trong hệ thống phức tạp, phân tán
Simulate sự cố & điều tra qua dashboard/log	- Tạo lỗi 500, memory spike, database chậm- Điều tra từ log, metric và trace	Khả năng “runbook thinking”, critical incident handling
Log rotation và log retention	- Cấu hình xoay log theo dung lượng/thời gian- Tự động xoá log cũ	Tránh đầy đĩa, tiết kiệm chi phí
Build dashboard theo business KPI	- Theo dõi signup rate, success login, order success- Biến log/metric thành dashboard	Monitoring hướng business chứ không chỉ tech
Kết hợp SLO/SLI/SLA để alert thông minh hơn	- Định nghĩa SLA 99.9%, alert nếu vượt error budget- Sử dụng Prometheus hoặc Grafana để tính toán	Chuyển từ reactive → proactive alerting
Export hệ thống metric custom từ ứng dụng (Prometheus Exporter)	- Expose /metrics endpoint- Tự ghi nhận request count, processing time, error rate	Đo chi tiết hành vi hệ thống
Monitor hạ tầng (CPU, RAM, Disk, Network)	- Node Exporter, EC2 metrics, CloudWatch, etc.- Cảnh báo khi vượt ngưỡng	Đảm bảo toàn hệ thống chứ không chỉ ứng dụng
Thực hiện “Post-Mortem” sau 1 sự cố mô phỏng	- Mô phỏng bug production (ex: memory leak)- Viết lại incident timeline, nguyên nhân gốc, giải pháp- Lưu bài học	Kỹ năng rất quan trọng ở cấp PrincipalTruyền lại văn hóa học từ lỗi
Phân quyền access log/metric (multi-team)	- Phân quyền Grafana dashboard / Kibana view theo role- Giới hạn log sensitive	Bảo mật & kiểm soát quyền truy cập observability tools
Kết hợp CI/CD pipeline với monitoring	- Gửi notification nếu deploy thành công/thất bại- Monitor ứng dụng sau mỗi deploy (canary checks)	CI/CD kết hợp observability để tăng độ an toàn khi release
Tạo hệ thống "Chaos Engineering" cơ bản (ex: kube-monkey / stress test)	- Gây lỗi CPU, kill service, network latency- Theo dõi khả năng recover / auto-heal qua monitoring	Kiểm tra thực tế resilience và phản ứng của hệ thống

## Microservices

- [Service Discovery & Load Balancing](./1.0.0/8hy0x3.md)
- API Gateway Implementation	
    - Xây dựng API Gateway cho hệ thống microservices- Cài đặt authentication, rate limiting	Tập trung bảo mật, quản lý truy cập và giới hạn yêu cầu
Inter-service Communication: Synchronous vs Async	- Triển khai REST hoặc gRPC cho sync- Triển khai message queue (Kafka, RabbitMQ) cho async communication	Hiểu trade-offs giữa sync và async, thiết kế hệ thống linh hoạt
Distributed Transaction & Saga Pattern	- Thiết kế xử lý giao dịch phân tán với Saga- Thực hành compensating transactions	Đảm bảo consistency và reliability trong microservices
Circuit Breaker & Retry Mechanism	- Triển khai circuit breaker (Resilience4j, Hystrix)- Thiết kế retry logic cho call giữa các service	Tăng resilience, xử lý lỗi mạng và service downtime
Service Mesh Introduction	- Cài đặt Istio hoặc Linkerd- Quản lý traffic, observability, và bảo mật microservices	Nâng cao khả năng quản trị microservices phức tạp
Centralized Logging & Distributed Tracing	- Thiết lập ELK stack (Elasticsearch, Logstash, Kibana)- Triển khai tracing với Jaeger hoặc Zipkin	Giám sát và debug toàn bộ luồng request trong microservices
Security in Microservices	- Thực hiện OAuth2 / JWT authentication across services- Thiết kế authorization theo role	Đảm bảo bảo mật xuyên suốt hệ thống
Configuration Management & Secrets Handling	- Sử dụng Spring Cloud Config / Consul- Quản lý secret bằng Vault hoặc AWS Secrets Manager	Quản lý cấu hình tập trung và bảo mật thông tin nhạy cảm
Scaling Microservices & Auto-scaling	- Thiết kế auto-scaling dựa trên metrics (CPU, request latency)- Sử dụng Kubernetes hoặc AWS ECS	Đáp ứng nhu cầu tăng giảm tài nguyên linh hoạt
Database per Service Pattern	- Thiết kế mô hình DB riêng cho mỗi microservice- Thực hành replication và data synchronization	Giảm coupling, tăng tính độc lập và scalability
Event-driven Architecture & CQRS	- Thiết kế event sourcing, publish-subscribe pattern- Phân tách đọc - ghi (CQRS)	Tăng hiệu suất và khả năng mở rộng hệ thống
Blue-Green Deployment & Canary Releases	- Triển khai blue-green hoặc canary để deploy microservices an toàn	Giảm downtime, kiểm soát rủi ro khi cập nhật
Handling Failures & Chaos Engineering	- Thiết kế test hệ thống với fault injection- Thực hành chaos testing để tăng robustness	Tăng cường khả năng chịu lỗi và phản ứng của hệ thống

## System Design

- [Xây dựng hệ thống cache phân tán cho trang web traffic cao](./1.0.0/7982s9.md)
    - Chọn loại cache (Redis, Memcached)
    - Thiết kế cache eviction policy
    - Cache invalidation	
    - Hiểu caching strategies, giảm tải DB, cải thiện performance
Thiết kế hệ thống bán vé sự kiện (ticketing system)	- Quản lý seat booking- Giải quyết race condition- Thiết kế API thanh toán	Hiểu concurrency, consistency, phân phối tải và fault tolerance
Thiết kế hệ thống file storage (ví dụ Dropbox, Google Drive)	- Thiết kế metadata service- Hỗ trợ upload/download file lớn, chunking, replication- Phân quyền access	Hệ thống phức tạp, storage và scalability
Thiết kế hệ thống tìm kiếm (search engine đơn giản)	- Xây dựng indexing, inverted index- Tối ưu truy vấn- Hỗ trợ pagination và ranking	Kỹ năng về data structures, performance, và scale
Thiết kế hệ thống recommendation (ví dụ Netflix)	- Thu thập dữ liệu user behavior- Thiết kế mô hình filtering và ranking- Streaming data pipelines	Hệ thống ML + big data cơ bản, scale, latency thấp
Thiết kế hệ thống order processing (e-commerce)	- Workflow order, inventory, payment- Xử lý thất bại giao dịch- Thiết kế event-driven architecture	Phân tách module, reliable messaging, eventual consistency
Thiết kế API Gateway & Rate Limiting	- Xây dựng gateway- Implement throttling, rate limit per user, IP	Bảo vệ hệ thống khỏi DDOS, nâng cao trải nghiệm người dùng
Thiết kế hệ thống logging và monitoring cho app phân tán	- Thu thập logs, metrics- Thiết kế hệ thống cảnh báo	Thiết kế observability từ đầu, giúp hệ thống dễ vận hành, debug
Thiết kế hệ thống xử lý dữ liệu thời gian thực (stream processing)	- Sử dụng Kafka/Fluentd, Spark Streaming- Thiết kế pipeline xử lý data	Kỹ năng xử lý big data, real-time
Thiết kế hệ thống notification (email, SMS, push notification)	- Hỗ trợ gửi notification hàng loạt- Thiết kế retry và queue hệ thống	Hệ thống chịu lỗi, phân phối lớn, đảm bảo delivery
Thiết kế hệ thống phân phối tải (load balancer)	- Xây dựng layer phân phối request- Health check, failover, sticky session	Hiểu cân bằng tải, đảm bảo uptime cao
Thiết kế hệ thống scale horizontally và vertically	- Thiết kế theo microservices- Scale DB và cache- Data sharding	Khả năng mở rộng hệ thống quy mô lớn
Thiết kế disaster recovery & backup system	- Lên kế hoạch backup DB, file- Thiết kế failover	Đảm bảo business continuity, giảm downtime

## [Browser - JS - Nodejs](./1.0.0/3q2t07.md)
