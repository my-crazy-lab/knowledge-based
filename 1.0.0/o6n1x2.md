...
## Các thuộc tính của một thuật toán đồng thuận (Consensus algorithm):
...
- ZooKeeper được mô phỏng theo dịch vụ khoá Chubby của Google, các tính năng khác rất hữu ích khi xây dựng các hệ thống phân tán:
    ...
    - Thứ tự tổng thể của các thao tác (**Total ordering of operations**)
        - Fecing token là một số tăng dần mỗi lần khoá được cấp. ZooKeeper cung cấp điều này bằng cách sắp xếp hoàn toàn tất cả các thao tác và gán cho mỗi thao tác một ID giao dịch (zxid) và số phiên bản (cversion) tăng dần.
